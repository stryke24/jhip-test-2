...
import ca.stryke.jhiptest2.domain.Job;
import ca.stryke.jhiptest2.repository.JobRepository;
...
/**
 * REST controller for managing {@link ca.stryke.jhiptest2.domain.Job}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class JobResource {
    ...
    public ResponseEntity<Job> partialUpdateJob(@RequestBody Job job) throws URISyntaxException {
        log.debug("REST request to update Job partially : {}", job);
        if (job.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }

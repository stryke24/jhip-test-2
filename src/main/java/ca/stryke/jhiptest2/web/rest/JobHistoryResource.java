...
import ca.stryke.jhiptest2.domain.JobHistory;
import ca.stryke.jhiptest2.service.JobHistoryService;
...
/**
 * REST controller for managing {@link ca.stryke.jhiptest2.domain.JobHistory}.
 */
@RestController
@RequestMapping("/api")
public class JobHistoryResource {
    ...
    public ResponseEntity<JobHistory> partialUpdateJobHistory(@RequestBody JobHistory jobHistory) throws URISyntaxException {
        log.debug("REST request to update JobHistory partially : {}", jobHistory);
        if (jobHistory.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
